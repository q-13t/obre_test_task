...
const Quotes = ({ q }: { q: Quote[] }) => {
    ...
    const quotes = useMemo(() => {
        const filtered = q
            ...
            .sort((a, b) => { const key = FilterCriteriaMap[context.filterBy]; return context.order === Order.asc ? a[key] - b[key] : b[key] - a[key]; });

        context.setTotalPages(Math.ceil(filtered.length / context.limit));

        return filtered.slice((context.page - 1) * context.limit, context.page * context.limit);
    }, [context.query, context.filterBy, context.order, context.limit, context.page, q]);
    ...
    const { subTotal, VAT, total, deposit, outstanding, profit } = useMemo(() => {
        ...
        return { subTotal: format(sub), VAT: format(vat), total: format(total), deposit: format(deposit), outstanding: format(outstanding), profit: format(profit) };
    }, [quotes]);
